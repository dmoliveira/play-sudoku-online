...
- Sudoku resume v2 and Suguru resume v3 remain version-compatible with one optional fail-closed `focusLaunchId`; Daily result v1, Weekly, Cage Garden, stats, and practice rotation schemas do not change; the additive focus-results v1 ledger is independently fail-closed.
...
- Focus launch/results: exact ordinary provenance, Classic target band, selectable resolution, explicit-launch completion/reload fall-through, ordinary-provenance non-credit, focus-ledger normalization/storage failure, resume recovery, no special-source credit, and practice-rotation bytes unchanged. A preseeded disabled-focus resume restores exactly while Compass falls through and practice excludes it.
